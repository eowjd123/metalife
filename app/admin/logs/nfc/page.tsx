...
export default function NFCLogPage() {
  ...
  // 검색 상태
  const [searchNfcKey, setSearchNfcKey] = useState('')
  ...
  const loadLogs = async (page: number = 1) => {
    ...
    try {
      ...
      if (searchNfcKey) params.append('nfcKey', searchNfcKey)
      if (searchUserid) params.append('nfcUserid', searchUserid)
      if (searchUsername) params.append('nfcUsername', searchUsername)
      if (dateStart) params.append('dateStart', dateStart)
      if (dateEnd) params.append('dateEnd', dateEnd)

      const response = await fetch(`/api/logs/nfc?${params.toString()}`)
      if (!response.ok) {
    ...
    <div className={styles.logsPage}>
      <div className={styles.header}>
        <div>
          ...
          <h1 className={styles.title}>NFC 로그인 로그</h1>
        </div>
      </div>
      ...
      {!isLoading && !error && (
        <div className={styles.filterBar}>
          <input
            ...
            placeholder="NFC 키 검색"
            value={searchNfcKey}
            onChange={(e) => setSearchNfcKey(e.target.value)}
          />
          <input
            className={styles.searchInput}
          ...
          <table className={styles.table}>
            ...
            <tbody>
              {logs.length > 0 ? (
                logs.map((log) => (
                  <tr key={log.id}>
                    <td className={styles.codeCell}>{log.nfcKey}</td>
                    <td className={styles.codeCell}>{log.nfcUserid}</td>
                    <td>{log.nfcUsername}</td>
