...
export default function MonthlyAccessStatusPage() {
  const [selectedPeriod, setSelectedPeriod] = useState<string>('1개월')
  const [startDate, setStartDate] = useState<string>('')
  const [endDate, setEndDate] = useState<string>('')
  const [accessData, setAccessData] = useState<Array<{ month: string; monthNum: number; count: number }>>([])
  ...
  useEffect(() => {
    ...
    start.setMonth(today.getMonth() - 11) // 기본 12개월
  ...
  const loadData = async (start: string, end: string) => {
    ...
    try {
      const response = await fetch(
        `/api/access-statistics/monthly?startDate=${start}&endDate=${end}`
      )
      if (!response.ok) {
  ...
  return (
    <div className={styles.accessStatus}>
      <h1 className={styles.title}>월별 접속 현황</h1>
      ...
      {!isLoading && !error && (
        <div className={styles.chartContainer}>
          <div className={styles.chartTitle}>월별 접속자 수</div>
          {accessData.length > 0 ? (
            <>
              <div className={styles.chart}>
                <div className={styles.chartYAxis}>
                  {(() => {
                    ...
                    const maxY = Math.ceil(maxCount / 2000) * 2000
                    const steps = 5
