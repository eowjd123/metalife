...
export default function AccessStatusPage() {
  const [selectedPeriod, setSelectedPeriod] = useState<string>('7일')
  const [startDate, setStartDate] = useState<string>('')
  const [endDate, setEndDate] = useState<string>('')
  const [accessData, setAccessData] = useState<Array<{ date: string; day: number; count: number }>>([])
  ...
  useEffect(() => {
    ...
    start.setDate(today.getDate() - 6) // 기본 7일
  ...
  const loadData = async (start: string, end: string) => {
    ...
    try {
      const response = await fetch(
        `/api/access-statistics/daily?startDate=${start}&endDate=${end}`
      )
      if (!response.ok) {
  ...
  return (
    <div className={styles.accessStatus}>
      <h1 className={styles.title}>일별 접속 현황</h1>
      ...
      {!isLoading && !error && (
        <div className={styles.chartContainer}>
          <div className={styles.chartTitle}>일별 접속자 수</div>
          {accessData.length > 0 ? (
            <>
