...
export default function MemberTraceLogPage() {
  const [logs, setLogs] = useState<MemberTraceLog[]>([])
  ...
  // 검색 상태
  const [searchMemberIdx, setSearchMemberIdx] = useState('')
  ...
  const loadLogs = async (page: number = 1) => {
    ...
    try {
      ...
      if (searchMemberIdx) params.append('memberIdx', searchMemberIdx)
      if (searchDomain) params.append('traceDomain', searchDomain)
      if (dateStart) params.append('dateStart', dateStart)
      if (dateEnd) params.append('dateEnd', dateEnd)

      const response = await fetch(`/api/logs/member-trace?${params.toString()}`)
      if (!response.ok) {
  ...
  // 통계 로드
  useEffect(() => {
    fetch('/api/logs/member-trace?limit=1')
      .then(res => res.json())
      .then(data => {
        if (data.stats) {
          setStats(data.stats)
    ...
    <div className={styles.logsPage}>
      <div className={styles.header}>
        <div>
          ...
          <h1 className={styles.title}>회원 추적 로그</h1>
        </div>
      </div>
      ...
      {!isLoading && !error && (
        <div className={styles.filterBar}>
          <input
            ...
            placeholder="회원 IDX 검색"
            value={searchMemberIdx}
            onChange={(e) => setSearchMemberIdx(e.target.value)}
          />
          <input
            className={styles.searchInput}
            placeholder="도메인 검색"
          ...
          <table className={styles.table}>
            ...
            <tbody>
              {logs.length > 0 ? (
                logs.map((log) => (
                  <tr key={log.id}>
                    <td className={styles.codeCell}>{log.memberId || log.memberIdx || '-'}</td>
                    <td>{log.memberName || '-'}</td>
                    <td>{log.traceDomain}</td>
