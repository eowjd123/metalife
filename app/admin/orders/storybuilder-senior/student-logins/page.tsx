...
export default function StudentLoginPage() {
  const [logs, setLogs] = useState<StudentLogin[]>([])
  ...
  const loadLogs = async (page: number = 1) => {
    ...
    try {
      ...
      if (searchMemberName) params.append('memberName', searchMemberName)
      if (searchDomain) params.append('loginDomain', searchDomain)
      if (dateStart) params.append('dateStart', dateStart)
      if (dateEnd) params.append('dateEnd', dateEnd)

      const response = await fetch(`/api/student-logins?${params.toString()}`)
      if (!response.ok) {
  ...
  // 통계 로드
  useEffect(() => {
    fetch('/api/student-logins?limit=1')
      .then(res => res.json())
      .then(data => {
        if (data.stats) {
          setStats(data.stats)
  ...
  return (
    <div className={styles.page}>
      <div className={styles.header}>
        <div>
          <h1 className={styles.title}>스토리빌더 시니어 · 수강생 로그인 이력</h1>
          <p className={styles.subtitle}>수강생의 로그인 이력을 추적하고 확인합니다.</p>
        </div>
        <div className={styles.actions}>
          <button className={styles.secondaryButton}>로그 내보내기</button>
        </div>
      </div>
      ...
      {!isLoading && !error && (
        <div className={styles.statsGrid}>
          ...
          <div className={styles.statCard}>
            ...
            <div className={styles.statContent}>
              <h3>오늘</h3>
              <p className={styles.statNumber}>{stats.today.toLocaleString()}</p>
            </div>
          </div>
          <div className={styles.statCard}>
            <div className={styles.statIcon} style={{ background: 'var(--primary-100)' }}>
            ...
            <div className={styles.statContent}>
              <h3>이번 주</h3>
              <p className={styles.statNumber}>{stats.thisWeek.toLocaleString()}</p>
            </div>
          </div>
          <div className={styles.statCard}>
            ...
            <div className={styles.statContent}>
              <h3>이번 달</h3>
              <p className={styles.statNumber}>{stats.thisMonth.toLocaleString()}</p>
            </div>
          </div>
        </div>
      )}
